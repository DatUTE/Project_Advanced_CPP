...
void NhapThongTinCaiDat_Display()
{
	...
	do {
		...
		if (listCar.size() > 0)
		{
			n = check_personalKey(tempCar->getPersonalKey());	//check if userkey existed or not
			if (n == listCar.size()) {
			...
			listCar[n]->getDisplay()->nhapThongTin();	// call Sound object from Setting obj and access nhapThongTin_Display()
		}
		else
		{
...
void NhapThongTinCaiDat_Sound()
{
	...
	do {
		...
		int m;
		if (listCar.size() > 0) {
			m = check_personalKey(tempCar->getPersonalKey());	//check if userkey existed or not
			if (m == listCar.size()) {
				listCar.push_back(tempCar);
			}
			else {
			...
			listCar[m]->getSound()->nhapThongTin();	// call Sound object from Setting obj and access nhapThongTin_Display()
		}
		else {
			cout << "This is the first car\n";
...
void XuatThongTinCaiDat_Sound() {
	system("cls");
	sort(listCar.begin(), listCar.end(), sortName_per);
	cout << "sort by name" << endl;
	cout << "---Sound setting---\n";
	cout <<setw(5) << "Number" << setw(10) << "Car Name" << setw(10) << "Email" << setw(13) << "MSC" << setw(7) << "ODO" << setw(12) << "SEVICES" << setw(15) << "Media" << setw(10) << "Call" << setw(10) << "Navi" << setw(10) << "Notify" << endl;
	for (int i = 0; i < listCar.size(); i++) {
		cout << setw(3) << i+1;
...
void XuatThongTinCaiDat_General() {
	...
	cout <<setw(5) << "Number" << setw(10) << "Car Name" << setw(10) << "Email" << setw(13) << "MSC" << setw(7) << "ODO" << setw(12) << "SEVICES" << setw(17) << "TimeZone" << setw(14) << "Language" << endl;
	for (int i = 0; i < listCar.size(); i++)
	{
		cout << setw(3) << i+1;
		listCar[i]->xuatThongTin();					// output personal information
...
void XuatThongTinCaiDat_Display() {
	...
	cout <<setw(5) << "Number" << setw(10) << "Car Name" << setw(10) << "Email" << setw(13) << "MSC" << setw(7) << "ODO" << setw(12) << "SEVICES" << setw(15) << "Light" << setw(10) << "Taplo" << setw(10) << "Screen" << endl;
	for (int i = 0; i < listCar.size(); i++)
	{
		cout << setw(3) << i +1;
...
void XuatTatCaThongTinCaiDat() {
	...
	do
	{
		...
		case 1:
		{
			...
			sort(listCar.begin(), listCar.end(), sortName_per);		// sort data by name
			for (int i = 0; i < listCar.size(); i++)
			{
				cout << "--------------------------------" << endl;
				cout << "Chiec xe thu: " << i + 1 << endl;
				cout << setw(12) << "Car Name" << setw(10) << "Email" << setw(13) << "MCS" << setw(8) << "ODO" << setw(10) << "Service" << endl;
				listCar[i]->xuatThongTin();
				cout << endl;
		...
		case 2:
		{
			...
			sort(listCar.begin(), listCar.end(), sortNumber_per);	// sort data by mscn
			for (int i = 0; i < listCar.size(); i++)
			{
				cout << "--------------------------------" << endl;
				cout << "Chiec xe thu: " << i + 1 << endl;
	...
	try {
		if (file1.is_open()) {
			for (int i = 0; i < listCar.size(); i++) { //write all data of obj elements: personal info, general, sound,display into file
				file1 << "Common: " << listCar[i]->getCarName() << "," << listCar[i]->getEmail() << "," << listCar[i]->getPersonalKey() << "," << listCar[i]->getODO() << "," << listCar[i]->getServiceRemind() << " ; ";
				file1 << "General: " << listCar[i]->getGeneral()->get_timeZone() << " , " << listCar[i]->getGeneral()->get_language() << " ; ";
				file1 << "Sound: " << listCar[i]->getSound()->get_media_level() << "," << listCar[i]->getSound()->get_call_level() << "," << listCar[i]->getSound()->get_navi_level() << "," << listCar[i]->getSound()->get_notification_level() << " ; ";
				file1 << "Display: " << listCar[i]->getDisplay()->get_light_level() << "," << listCar[i]->getDisplay()->get_screen_light_level() << "," << listCar[i]->getDisplay()->get_taplo_light_level() << " ;" << endl;
			}
		}
		else {
			throw exception(); //throw out the warning
	...
	try {
		if (file1.is_open()) {
			while (getline(file1, s)) {
				...
				listCar.push_back(temp); // add it into the array elements_ of private of List
			}
		}
		else {
