...
{
	cout << setw(10) << getCarName() << setw(18) << getEmail() << setw(10) << getPersonalKey() << setw(5) << getODO() << setw(10) << getServiceRemind();
}
...
void Setting::nhapThongTin()
{
	...
	while (1)
	{
		...
		{
			cout << "Email Nhap Vao Phai Theo Cu Phap email@abc.xyz" << endl;
		}
		else
		{
			break;
	...
	while (1)
	{
		...
		{
			cout << "Ma So Nhap Vao Phai Du 8 So" << endl;
		}
		else
		{
			{
...
void Setting:: Swap(int i, int j)
{
	string a;
	string elements_[100];
	a = elements_[i];
